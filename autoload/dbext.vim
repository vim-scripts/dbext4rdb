...
"                        \ begin call ora_login();call v$nls_set_func();end;\n" )
"                        set display no row counter\n" .
                        "\ attach 'filename  . s:DB_option(' ', s:DB_get("dbname"), '''')  . dbext#DB_getWType("cmd_terminator") .
    elseif a:name ==# "RDB_cmd_header"
      let host=s:DB_get("host")
      let srvname=s:DB_get("srvname")
      let user=s:DB_get("user")
      let passwd=s:DB_get("passwd")
      call s:DB_warningMsg("host set to '" . host . "'")
      call s:DB_warningMsg("srvname set to '" . srvname . "'")
      call s:DB_warningMsg("user set to '" . user . "'")
      call s:DB_warningMsg("passwd set to '" . passwd . "'")
      if host != ''
        let srvname = host
      endif
      let sup = ''
      if srvname != ''
        ...
        call s:DB_warningMsg("sup set to " . sup)
      endif
      return (exists("g:dbext_default_RDB_cmd_header")?g:dbext_default_RDB_cmd_header.'':
                \ s:DB_option('attach ''filename ', sup . s:DB_get("dbname"), '''')  . dbext#DB_getWType("cmd_terminator") .
...
function! s:DB_RDB_getDictionaryTable() "{{{
    let result = s:DB_RDB_execSql(
                ...
                \ )
        " call s:DB_warningMsg( 'dbext:debug DB_RDB_getDictionaryTable.result ' . result )
        " call s:DB_warningMsg( 'dbext:debug DB_RDB_getDictionaryTable.result after strip ' . s:DB_RDB_stripHeaderFooter(result) )
    return s:DB_RDB_stripHeaderFooter(result)
endfunction "}}}
...
function! s:DB_RDB_getListColumn(table_name) "{{{
    ...
    let result = s:DB_RDB_execSql( query )
        "call s:DB_warningMsg( 'dbext:debug DB_RDB_getListColumn.result ' . result )
    return s:DB_RDB_stripHeaderFooter(result)
endfunction "}}}
...
function! dbext#DB_settingsComplete(ArgLead, CmdLine, CursorPos)
